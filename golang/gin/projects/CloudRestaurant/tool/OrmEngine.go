...
func OrmEngine(cfg *Config) (*Orm, error) {
	...
	engine.ShowSQL(database.ShowSql) // 表示我们在执行数据库操作的时候，是否输出一个SQL语句

	// 将model同步映射成为数据库中的member表
	if err := engine.Sync2(
		new(model.SmsCode),      // 发送短信模型
		new(model.Member),       // 用户模型
		new(model.FoodCategory), // 食品类别模型
		new(model.Shop),         // 商户模型
		new(model.Service),      // 服务模型
		new(model.ShopService),  // 商户-服务关系模型
		new(model.Goods),        // 商品模型
	); err != nil { // 将SmsCode的结构体定义转变成了数据库当中的一张表
		return nil, err
	}
...
func InitServiceData() {
	services := []model.Service{
		model.Service{Id: 1, Name: "外卖保", Description: "已加入“外卖保”计划，食品安全有保障", IconName: "保", IconColor: "999999"},
		model.Service{Id: 2, Name: "准时达", Description: "准时必达，超时秒赔", IconName: "准", IconColor: "999999"},
		model.Service{Id: 3, Name: "开发票", Description: "该商家支持开发票，请在下单时填写好发票抬头", IconName: "票", IconColor: "999999"},
	}
